...
BinaCPP::get_price( const char *symbol )
{
	...
	string url(BINANCE_HOST);  
	url += "/ticker/price?symbol="+str_symbol;
...
{	
	...
	Json::Value json_res;
	BinaCPP::get_serverTime(json_res);
//	querystring.append( to_string( get_current_ms_epoch() ) );
	std::string lo = json_res["serverTime"].asString().c_str();;
	querystring.append(lo);
